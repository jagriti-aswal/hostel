...
const AdminDashboard: React.FC = () => {
  ...
  const filteredStudents = useMemo(() => {
    ...
    return students.filter((s) => {
      ...
      const isPresent = attendance.has(s._id);
      const isOnLeave = s.isOnLeave;

      if (filter === "present") return matchesSearch && isPresent;
      if (filter === "absent") return matchesSearch && !isPresent && !isOnLeave;
  ...
  const total = students.length;
  const absentCount = total - presentCount - leaveCount;
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
          ...
          <Stat label="Present" value={presentCount} icon={<CheckCircle2 />} />
          <Stat label="On Leave" value={leaveCount} icon={<Calendar />} />
          <Stat label="Absent" value={absentCount} icon={<XCircle />} />
          <Stat label="Rate" value={`${attendanceRate}%`} icon={<Clock />} />
        </div>
